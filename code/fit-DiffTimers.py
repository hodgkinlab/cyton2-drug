...
def fit(inputs):
	...
	if key == 'Fig 2-3 DV23_007 OTI BimKO corrected':
		pars = {  # Initial values
			'mDiv0': 30, 'sDiv0': 0.2,         # Time to first division
			'mDD': 60.44, 'sDD': 12.033,       # Time to division destiny (Fitted from No Drug)
			'mDie': a1_mDie, 'sDie': a1_sDie,  # Time to death
			'b': 10							   # Subseqeunt division time
		}
	elif key == 'DV24_003 Rapa+DEX':
		pars = {  # Initial values
			'mDiv0': 30, 'sDiv0': 0.2,
			'mDD': 57.38, 'sDD': 12.647,       # Fitted from No Drug
			'mDie': a1_mDie, 'sDie': a1_sDie,
			'b': 10
		}
	elif key == 'Fig 2-3 DV24_001 Dex MPA OTI BimKO':
		pars = {  # Initial values
			'mDiv0': 30, 'sDiv0': 0.2,
			'mDD': 61.40, 'sDD': 13.532,       # Fitted from No Drug
			'mDie': a1_mDie, 'sDie': a1_sDie,
			'b': 10
		}
...
if __name__ == "__main__":
	...
	########### DATA ###########
	DATA_FILES = [
		## Figure 3 - Different timers
		'Fig 2-3 DV23_007 OTI BimKO corrected.xlsx',
		'DV24_003 Rapa+DEX.xlsx',  # Repeat of DV23_007 DEX + RAPA

		## Supplementary Figure - Different timers
		'Fig 2-3 DV24_001 Dex MPA OTI BimKO.xlsx'
	]
