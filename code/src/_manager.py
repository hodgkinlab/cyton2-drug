		...
		for itpt in range(len(filtered_cell_gens_reps[icnd])):
			for irep, l in enumerate(filtered_cell_gens_reps[icnd][itpt]):
				for igen, datum in enumerate(l):
					resorted_data[icnd][itpt][igen].append(datum)
			tmp = []
			for idx, datum in enumerate(resorted_data[icnd][itpt]):
				if len(datum) == 0: # ignore "over-estimated" generation list
					continue

				_sem = sps.sem(datum)
				if np.isnan(_sem):  # check if there's only one replicate
					tmp.append(0)
				else:
					tmp.append(_sem)
